...
server <- function(input, output, session) {
  ...
  output$filtered_data <- renderDataTable({
    filtered_data <- df()[df()$cohort_group == input$cohort_group &
      ...
      select(
        lat, lon, school_name, cohort_group, cohort_start,
  ...
  observe({
    ...
    color_palette <-colorNumeric(palette = c('red', 'dark green'),
                                 c(0,100))
    ...
    leafletProxy("my_map", data = filtered_data()) %>%
      addCircleMarkers(
        ...
        label = paste0(filtered_data()$school_name, " ", round(outcome_variable),
                       "%"),
        color = ~color_palette(outcome_variable)) %>% 
      addLegend('bottomleft', pal = color_palette, bins = c(0,20,40,60,80,100),
                opacity = 1, values = c(0,100),
                title = paste(input$cohort_type, "<br>",full_name_outcome_variable,
